 ...
 */
import { BufferAttribute, BufferGeometry, DoubleSide, Group, Mesh, ShaderMaterial, Vector3, } from 'threepipe';
...
export class BowArrowTrails {
    ...
    tangentScratch = new Vector3();
    cameraDirectionScratch = new Vector3();
    sideScratch = new Vector3();
    ...
    makeSlot() {
        const positions = new Float32Array(TRAIL_POINT_CAPACITY * VERTICES_PER_POINT * POSITION_COMPONENTS);
        ...
        for (let i = 0; i < TRAIL_POINT_CAPACITY - 1; i += 1) {
            ...
            indices.push(firstVertex, firstVertex + 1, firstVertex + THIRD_INDEX_OFFSET, firstVertex + 1, firstVertex + FOURTH_INDEX_OFFSET, firstVertex + THIRD_INDEX_OFFSET);
        }
    ...
    updateSlot(slot, time, cameraPosition) {
    ...
    findVisibleStartIndex(slot, time) {
        let startIndex = 0;
    ...
    writePointVertices(slot, pointIndex, visibleIndex, startIndex, time, cameraPosition) {
