...
function isPlayerNameRecord(value) {
...
function removeStoredPlayerName() {
    try {
...
export function sanitizePlayerNameInput(value) {
    ...
    const withoutControlCharacters = Array.from(value)
        ...
        return (characterCode >= FIRST_VISIBLE_CHARACTER_CODE && characterCode !== DELETE_CHARACTER_CODE);
    })
        .join('')
        .trim();
 ...
 */
export function readPlayerName(nowMs = Date.now()) {
    try {
 ...
 */
export function savePlayerName(value, nowMs = Date.now()) {
    const name = sanitizePlayerNameInput(value) || cleanPlayerName(value);
    const record = { name, expiresAt: nowMs + SEVEN_DAYS_MS };
    try {
