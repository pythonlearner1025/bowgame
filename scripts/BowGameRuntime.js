...
function arrowModel() { const g = new Group(); const wood = MAT(0xaa772d), iron = MAT(0xc3c6be, .25), feather = MAT(0x6d5732); stick(g, new Vector3(0, 0, .28), new Vector3(0, 0, -.71), .0035, wood); const tip = mesh(new CylinderGeometry(0, .006, .06, 4), iron, g, 0, 0, -.735); tip.rotation.x = -Math.PI / 2; for (let i = 0; i < 3; i++) {
    const f = mesh(new BoxGeometry(.042, .003, .15), feather, g, 0, 0, .2);
...
export class BowGameRuntime {
    ...
    inspect(params) {
        ...
        if (flightSeconds !== undefined && (!Number.isFinite(flightSeconds) || flightSeconds < 0 || flightSeconds > 1))
        ...
        if (!['first-person', 'character'].includes(view) || !Number.isFinite(draw) || draw < 0 || draw > 1 || !Number.isFinite(release) || release < -1 || release > 2 || !Number.isFinite(orbit) || Math.abs(orbit) > 180)
            throw new Error('Invalid bow inspection view, draw, release or orbit');
        if (referenceTime !== undefined && (!Number.isFinite(referenceTime) || referenceTime < 0 || referenceTime > 10))
    ...
    async start() {
        ...
        this.cameraRestore = { position: camera.position.clone(), quaternion: camera.quaternion.clone(), target: camera.target?.clone(), fov: camera.fov, controls: camera.controls?.enabled };
        ...
        scene.fog = new FogExp2(0xa5b3b4, .012);
        ...
        Object.assign(sun.shadow.camera, { left: -34, right: 34, top: 34, bottom: -34, near: .5, far: 95 });
    ...
    stop() {
        ...
        this.arrows = [];
        this.bots = [];
        this.hidden.forEach(s => s.object.visible = s.visible);
        this.hidden = [];
        const viewer = this.viewer;
        if (viewer && this.cameraRestore) {
            const c = viewer.scene.mainCamera, s = this.cameraRestore;
            c.position.copy(s.position);
            c.quaternion.copy(s.quaternion);
            if (s.target)
                c.target?.copy(s.target);
            if (c.controls)
                c.controls.enabled = s.controls;
            c.fov = s.fov;
    ...
    createBot(i) {
        ...
        const g = human.root;
        g.name = ['ASH', 'ROOK', 'VALE', 'FLINT', 'MOSS', 'BEAR'][i];
        const bow = bowModel();
    ...
    updateBotPose(b, draw, walk = 0, relaxed = false, release = -1) {
        ...
        poseHuman(b.human, pose.draw, walk, relaxed);
        b.bow.visible = b.heldArrow.visible = !relaxed;
        b.bow.position.set(-.22, 1.56, -.60);
        b.bow.rotation.set(-.10, .58, 0);
        if (release >= .15 && release < 1.05) {
            const t = (release - .15) / .9, reach = Math.sin(t * Math.PI);
            b.bow.position.y -= reach * .13;
            b.bow.rotation.z = -reach * .22;
            poseArm(b.human.right, new Vector3(.245, 1.44, 0), new Vector3(.45, 1.5 + reach * .15, .05), new Vector3(.13, 1.4 + reach * .48, -.1 + reach * .24));
        }
        animateBow(b.bow, pose.draw, pose.vibration);
        b.heldArrow.visible = !relaxed && pose.arrowVisible;
        const nock = bowNock(pose.draw).applyQuaternion(b.bow.quaternion).add(b.bow.position);
        if (!relaxed) {
            poseArm(b.human.left, new Vector3(-.245, 1.44, 0), new Vector3(-.29, 1.46, -.31), b.bow.position, b.bow.quaternion);
            if (release < .15 || release >= 1.05)
                poseArm(b.human.right, new Vector3(.245, 1.44, 0), new Vector3(.40 + pose.draw * .12, 1.38 + pose.draw * .08, -.12 + pose.draw * .2), nock, b.bow.quaternion);
        }
        b.heldArrow.position.copy(nock).add(new Vector3(0, 0, -.28).applyQuaternion(b.bow.quaternion));
        b.heldArrow.quaternion.copy(b.bow.quaternion);
    ...
    onKeyUp = (e) => { if (this.keys.delete(e.code)) {
        e.preventDefault();
        e.stopImmediatePropagation();
    } };
    onContext = (e) => e.preventDefault();
    onMouseDown = (e) => { if (!this.running || e.target !== this.viewer.canvas)
        return; e.preventDefault(); e.stopImmediatePropagation(); if (!this.active) {
        this.enter();
        return;
    } if (e.button === 0 && this.hp > 0 && !this.winner) {
    ...
    catch { } this.active = true; this.sounds ??= new BowAudio(); this.sounds.resume().catch(() => { }); };
    onMouseUp = (e) => {
        if (!this.running)
            return;
        if (e.button === 0) {
    ...
    onMouseMove = (e) => { if (!this.running || !this.active)
        return; if (document.pointerLockElement !== this.viewer.canvas && !(e.buttons & 1))
        return; this.yaw -= e.movementX * (this.aiming ? .0011 : .0018); this.pitch = Math.max(-1.25, Math.min(1.25, this.pitch - e.movementY * (this.aiming ? .0011 : .0018))); e.stopImmediatePropagation(); };
    onBlur = () => { this.keys.clear(); if (this.drawing && this.charge > 0) {
    ...
    firePlayer(pose, charge) {
        ...
        this.updateCamera();
        const camera = this.viewer.scene.mainCamera, tip = referenceArrow(pose).tip;
    ...
    step(dt) {
        ...
        if (this.cancelTime >= 0) {
            this.cancelTime += dt;
            if (this.cancelTime >= .3) {
                this.cancelTime = -1;
                this.cancelFrom = null;
            }
        }
        this.cooldown = Math.max(0, this.cooldown - dt);
        if (this.queuedDraw && this.cooldown <= 0 && this.cancelTime < 0 && this.hp > 0 && !this.winner) {
            this.drawing = true;
            this.charge = 0;
            this.queuedDraw = false;
        }
    ...
    stepBot(b, index, dt) {
        if (b.hp <= 0) {
            if (this.elapsed > b.respawn)
                this.spawnBot(b, index);
            return;
        }
        const target = this.player.clone().add(new Vector3(0, 1.28, 0)), origin = b.mesh.position.clone().add(new Vector3(0, 1.34, 0));
        const delta = target.clone().sub(origin), distance = delta.length();
        b.mesh.rotation.y = Math.atan2(-delta.x, -delta.z);
        b.cooldown -= dt;
        if (b.release >= 0) {
            b.release += dt;
            if (b.release >= 1.05)
        ...
        if (this.hp > 0 && !blocked && distance < 38 && b.cooldown <= 0) {
            b.draw += dt / 1.1;
            if (b.draw >= 1) {
                const level = this.config.difficulty, spread = level === 'easy' ? .10 : level === 'hard' ? .017 : .045;
                const speed = shotSpeed(.85);
                target.y += GRAVITY * .5 * Math.pow(distance / speed, 2);
                target.x += (Math.random() - .5) * distance * spread;
                target.y += (Math.random() - .5) * distance * spread;
                this.fire(origin, target.sub(origin).normalize(), index, .85);
                b.draw = 0;
                b.release = 0;
                b.cooldown = (level === 'easy' ? 3.4 : level === 'hard' ? 1.7 : 2.6) + Math.random();
            }
        }
        else
            b.draw = Math.max(0, b.draw - dt * 2);
    ...
    stepArrows(dt) {
        for (const arrow of [...this.arrows]) {
            ...
            for (const c of candidates) {
                if (c.hp <= 0)
                    continue;
                for (const [height, r, isHead] of [[1.65, .24, true], [1.05, .4, false], [.5, .29, false]]) {
                    const t = segmentSphere(from, to, c.position.clone().add(new Vector3(0, height, 0)), r);
                    if (t !== null && t < nearest) {
                        nearest = t;
                        victim = c.index;
                        head = isHead;
            ...
            arrow.position.copy(from).lerp(to, nearest);
            this.trails?.sample(arrow.trail, arrow.position, this.elapsed);
            arrow.mesh.position.copy(arrow.position).addScaledVector(arrow.velocity.clone().normalize(), -.765);
    ...
    damage(victim, damage, owner, head) {
        if (head && owner < 0 && victim >= 0)
            this.sounds?.headshotConfirm();
        if (victim === -1) {
            ...
            const b = this.bots[victim];
            b.hp = Math.max(0, b.hp - damage);
            this.hit = 1;
            this.message = `${head ? 'HEADSHOT' : 'HIT'}  −${damage}  ${b.name}`;
            this.messageUntil = this.elapsed + 1.7;
            if (!b.hp) {
                b.deaths++;
                b.respawn = this.elapsed + 3.5;
                b.mesh.visible = false;
                this.kills++;
                this.message = `${b.name} eliminated  +1`;
            }
        }
        if (this.kills >= this.config.scoreLimit)
            this.winner = 'YOU';
        const winningBot = this.bots.find(b => b.kills >= this.config.scoreLimit);
        if (winningBot)
    ...
    updateCamera() {
        ...
        if (this.preview?.view === 'character') {
            const c = this.viewer.scene.mainCamera, b = this.bots[0];
            this.bow.visible = this.arm.visible = this.hand.visible = this.heldArrow.visible = false;
            this.bots.forEach((v, i) => v.mesh.visible = i === 0);
            const angle = this.preview.orbit * Math.PI / 180;
            b.mesh.position.set(0, 0, 17);
            b.mesh.rotation.set(0, 0, 0);
            b.leftLeg.rotation.set(0, 0, 0);
            b.rightLeg.rotation.set(0, 0, 0);
            this.updateBotPose(b, this.preview.draw, 0, this.preview.draw === 0 && this.preview.release < 0, this.preview.release);
            c.position.set(Math.sin(angle) * 3.15, .98, 17 - Math.cos(angle) * 3.15);
            c.lookAt(0, .98, 17);
            c.target?.set(0, .98, 17);
            c.fov = this.preview.draw > 0 || this.preview.release >= 0 ? 48 : 40;
            c.updateProjectionMatrix?.();
            c.setDirty?.({ change: 'transform' });
            return;
        }
        if (this.preview)
        ...
        camera.position.copy(this.player).add(new Vector3(0, this.hp > 0 ? 1.66 : .55, 0));
        if (walk)
            camera.position.y += Math.sin(this.elapsed * 10) * .025;
        camera.rotation.set(this.pitch, this.yaw, 0, 'YXZ');
        const dir = new Vector3(0, 0, -1).applyQuaternion(camera.quaternion);
        camera.target?.copy(camera.position).add(dir);
        const wanted = 76;
        camera.fov = wanted;
        camera.updateProjectionMatrix?.();
        ...
        const pose = this.preview?.referenceTime !== undefined ? sampleReferenceTimeline(this.preview.referenceTime) : this.preview ? sampleReferenceAction(this.preview.draw, release, (this.preview.aim ?? this.aiming) ? 1 : 0) : this.sampleLivePose();
        this.posePhase = pose.phase;
        const localRotation = referenceRotation(pose), grip = referenceScreenPoint(...pose.grip), scale = .9;
        this.bow.position.copy(grip).applyQuaternion(q).add(camera.position);
        this.bow.quaternion.copy(q).multiply(localRotation);
        this.bow.scale.setScalar(scale);
        animateBow(this.bow, pose.tension, release >= 0 ? Math.sin(release * 115) * Math.exp(-release * 24) * .025 : 0);
        this.arm.position.copy(camera.position);
        this.hand.position.copy(camera.position);
        this.arm.quaternion.copy(q);
        this.hand.quaternion.copy(q);
        const arrow = referenceArrow(pose), nock = arrow.nock;
        if (pose.arrowVisible && pose.arrowSeated) {
            const localNock = nock.clone().sub(grip).applyQuaternion(localRotation.clone().invert()).divideScalar(scale), string = this.bow.userData.bowString, points = string.geometry.getAttribute('position');
        ...
        if (this.leftArm)
            poseArm(this.leftArm, new Vector3(-.68, -.55, .12), new Vector3(-.54, -.42, -.35), grip, localRotation);
        if (this.rightArm && pose.rightVisible)
            poseArm(this.rightArm, new Vector3(.5, -.6, .08), new Vector3(.48, -.45, -.06), pull, new Quaternion().setFromAxisAngle(new Vector3(0, 0, 1), -.15));
        ...
        this.arm.visible = this.bow.visible = this.hp > 0;
        const arrowAnchor = nock;
        const arrowRotation = arrow.rotation;
        this.heldArrow.position.copy(arrowAnchor).add(new Vector3(0, 0, -.28 * scale).applyQuaternion(arrowRotation)).applyQuaternion(q).add(camera.position);
        this.heldArrow.quaternion.copy(q).multiply(arrowRotation);
        ...
        if (this.preview?.flightSeconds !== undefined) {
            ...
            if (this.preview.flightSide && this.arrows[0]) {
                ...
                this.bow.visible = this.arm.visible = this.hand.visible = false;
