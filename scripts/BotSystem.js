...
export class BotSystem {
    ...
    poseArms(bot, options) {
        ...
        if (options.release < 0.15 || options.release >= 1.05) {
            poseArm(bot.human.right, new Vector3(0.245, 1.44, 0), new Vector3(0.4 + options.draw * 0.12, 1.38 + options.draw * 0.08, -0.12 + options.draw * 0.2), options.nock, bot.bow.quaternion);
        }
    }
    ...
    move(bot, old, movement, dt) {
        if (!this.world.collision) {
            bot.mesh.position.copy(moveWithCover(old, movement.x, movement.z, this.getConfig().obstacles, 0.42));
            return;
        }
    ...
    applyEscapeMovement(bot, old, movement, dt) {
        if (!bot.escapeTarget) {
            return;
        }
        movement
    ...
    stepAttack(bot, options) {
        ...
        this.callbacks.fire(options.origin, options.target.sub(options.origin).normalize(), options.index, 0.85);
        bot.draw = 0;
        bot.release = 0;
