...
export class BowPerformance {
    ...
    record(at, dt, cpu, active) {
        this.samples.push({ at, dt, cpu, active, visible: document.visibilityState === 'visible', rendered: this.rendered - this.previousRendered });
        this.previousRendered = this.rendered;
        while (this.samples.length > 7200 || this.samples[0]?.at < at - 60000)
            this.samples.shift();
    }
    summary() {
        const frames = this.samples.filter(s => s.visible && s.dt > 0), active = frames.filter(s => s.active);
        const statistics = (list) => { const intervals = list.map(s => s.dt).sort((a, b) => a - b), cpu = list.map(s => s.cpu).sort((a, b) => a - b), sum = intervals.reduce((a, b) => a + b, 0), at = (a, p) => a[Math.min(a.length - 1, Math.floor(a.length * p))] ?? 0; const rendered = list.reduce((sum, s) => sum + s.rendered, 0); return { samples: list.length, renderedFrames: rendered, renderFps: sum ? Number((rendered * 1000 / sum).toFixed(1)) : 0, longFrames: list.filter(s => s.dt >= 250).length, maxFrameMs: Number((intervals[intervals.length - 1] ?? 0).toFixed(2)), seconds: Number((sum / 1000).toFixed(2)), fps: sum ? Number((list.length * 1000 / sum).toFixed(1)) : 0, frameMsP50: Number(at(intervals, .5).toFixed(2)), frameMsP95: Number(at(intervals, .95).toFixed(2)), gameCpuMsP50: Number(at(cpu, .5).toFixed(2)), gameCpuMsP95: Number(at(cpu, .95).toFixed(2)) }; };
    ...
    dispose() { this.viewer.removeEventListener('preRender', this.beforeRender); this.viewer.removeEventListener('postRender', this.afterRender); if (this.query) {
        this.gl.endQuery(this.extension.TIME_ELAPSED_EXT);
        this.gl.deleteQuery(this.query);
    } for (const query of this.pending)
        this.gl.deleteQuery(query); this.pending = []; }
}
