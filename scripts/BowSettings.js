...
function isBowSettingsRecord(value) {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}
function isBindingRecord(value) {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}
function isNonEmptyCodeList(value) {
...
function readUnboundActions(value) {
    if (!Array.isArray(value)) {
        return [];
    }
    return value.filter((action) => typeof action === 'string' && BOW_ACTIONS.includes(action));
}
function removeBindingConflicts(bindings) {
...
function clampMouseSensitivity(value) {
    return Math.max(MIN_MOUSE_SENSITIVITY_MULTIPLIER, Math.min(MAX_MOUSE_SENSITIVITY_MULTIPLIER, value));
}
...
export class BowSettings {
    ...
    isMouseYInverted = false;
    mouseSensitivityMultiplier = DEFAULT_MOUSE_SENSITIVITY_MULTIPLIER;
     ...
     */
    isAnyMovementActive(keys) {
        return (this.isActionActive(keys, 'moveForward') ||
    ...
    rebind(action, code) {
        ...
        for (const otherAction of BOW_ACTIONS) {
            this.bindings[otherAction] = this.bindings[otherAction].filter((boundCode) => boundCode !== code);
        }
    ...
    readStoredSettings() {
        try {
            ...
            if (typeof storedValue.mouseSensitivity === 'number' &&
