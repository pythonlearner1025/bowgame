...
export class CombatRules {
     ...
     */
    constructor(state, getConfig, isOnline, callbacks) {
        this.state = state;
    ...
    restart() {
        ...
        this.state.player.copy(this.isOnline()
    ...
    damageBot(victim, damage, isHead) {
    ...
    applyNetworkHit(message) {
        ...
        this.callbacks
            .getAudio()
            ?.impact(this.state.player.clone().add(new Vector3(0, message.head ? 1.65 : 1.05, 0)), message.head ? 'head' : 'body');
        if (this.state.hp > 0) {
