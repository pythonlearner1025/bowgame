...
export function shotSpeed(charge) { return 18 + Math.min(1, Math.max(0, charge)) * 38; }
export function shotDamage(charge, headshot = false) { return Math.round((24 + Math.min(1, Math.max(0, charge)) * 46) * (headshot ? 1.8 : 1)); }
/** Earliest segment/sphere intersection in [0,1]; swept tests prevent tunnelling. */
export function segmentSphere(a, b, c, radius) {
...
export function segmentCover(a, b, c) {
    ...
    if (aa < 1e-12) {
        ...
        if (d < 0)
            return null;
        lo = Math.max(lo, (-bb - Math.sqrt(d)) / (2 * aa));
        hi = Math.min(hi, (-bb + Math.sqrt(d)) / (2 * aa));
    }
    if (Math.abs(dy) < 1e-12) {
        ...
        const t0 = -a.y / dy, t1 = (c.height - a.y) / dy;
        lo = Math.max(lo, Math.min(t0, t1));
        hi = Math.min(hi, Math.max(t0, t1));
    }
    return lo <= hi ? lo : null;
}
export function moveWithCover(position, dx, dz, covers, radius = 0.38) {
