...
function replaceScreeGeometry(mesh) {
    ...
    for (let i = 0; i < positions.count; i += 1) {
        ...
        const largeDistortion = LARGE_DISTORTION_AMPLITUDE *
            Math.sin(x * LARGE_X_FREQUENCY + z * LARGE_Z_FREQUENCY) *
            Math.cos(y * LARGE_Y_FREQUENCY);
        const smallDistortion = SMALL_DISTORTION_AMPLITUDE *
...
function disposeBatch(batches, originals, replacements) {
    ...
    arena.traverseVisible((object) => {
        ...
        if (mesh.isInstancedMesh) {
            ...
            if (/^Forest[ _]scree$/.test(mesh.name)) {
                replacements.push(replaceScreeGeometry(mesh));
            }
            return;
        }
