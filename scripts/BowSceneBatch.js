...
export function batchBowScene(arena, runtime) {
    const originals = [], batches = [], geometries = [];
        ...
        if (mesh.isInstancedMesh) {
            if (/^Forest[ _]scree$/.test(mesh.name)) {
                ...
                for (let i = 0; i < p.count; i++) {
                    const x = p.getX(i), y = p.getY(i), z = p.getZ(i), d = 1 + .15 * Math.sin(x * 4 + z * 3) * Math.cos(y * 4) + .045 * Math.sin(x * 13 - z * 8) * Math.cos(y * 11);
                    p.setXYZ(i, x * d, y * d, z * d);
                }
                replacement.computeVertexNormals();
                geometries.push({ mesh: mesh, original: mesh.geometry, replacement });
    ...
    for (const meshes of groups.values()) {
        ...
        batch.name = 'Static arena batch: ' + first.name;
