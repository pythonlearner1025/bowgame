 ...
 */
function smoothAnatomyNormals(geometry) {
    geometry.computeVertexNormals();
    const positions = geometry.getAttribute('position');
    const normals = geometry.getAttribute('normal');
    const sums = new Map();
    const key = (i) => `${positions.getX(i).toFixed(5)},${positions.getY(i).toFixed(5)},${positions.getZ(i).toFixed(5)}`;
...
export function attachHumanAsset(human, index, source = asset) {
    ...
    if (data.eyes) {
        ...
        const eyes = new SkinnedMesh(eyeGeometry, new MeshStandardMaterial({ map: eyeTexture ?? null, roughness: 0.35 }));
    ...
    const solveArm = ({ side, goal, guide, isRelaxed, roll }) => {
        ...
        const upperRest = vectorFromArray(data.bones[lower].head).sub(vectorFromArray(data.bones[upper].head));
        const lowerRest = vectorFromArray(data.bones[wrist].head).sub(vectorFromArray(data.bones[lower].head));
        ...
        if (isRelaxed) {
            ...
            const gripOffset = side === 'L' ? new Vector3(-0.038, -0.026, 0.106) : new Vector3(0.01, -0.023, 0.145);
            wristOffset = gripOffset.applyQuaternion(roll);
        }
        const target = goal.clone().add(wristOffset);
        target.y += 0.04;
        const direction = target.clone().sub(shoulder), distance = Math.min(direction.length(), upperLength + lowerLength - 0.001);
        direction.normalize();
        const alongDistance = (upperLength * upperLength - lowerLength * lowerLength + distance * distance) /
            (2 * distance);
        const height = Math.sqrt(Math.max(0, upperLength * upperLength - alongDistance * alongDistance));
        const bend = guide
            .clone()
            .sub(shoulder)
        ...
        if (isRelaxed) {
            ...
            const direction = (side === 'L' ? new Vector3(0.03, 0.16, -1) : new Vector3(0.28, 0.1, -1)).normalize();
