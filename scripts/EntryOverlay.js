...
export class EntryOverlay {
     ...
     */
    constructor(viewer, state, getConfig, callbacks) {
        this.viewer = viewer;
        this.state = state;
    ...
    makeClearButton(nameField, input, hasStoredName) {
    ...
    enter() {
        ...
        this.state.sounds.resume().catch((error) => {
    ...
    updateOnline() {
        ...
        this.setStyle('nameField', 'display', this.state.winner || status === 'full' ? 'none' : 'block');
        this.setStyle('solo', 'display', isBlocked ? 'block' : 'none');
    }
    getOnlineDescription(status, limit) {
        if (status === 'full') {
    ...
    setOnlineButtonText(status, isBlocked) {
        let buttonText = 'ENTER ARENA';
        if (this.state.winner) {
    ...
    setDisabled(id, value) {
        ...
        this.values[key] = value;
        this.elements[id].disabled = value;
    }
