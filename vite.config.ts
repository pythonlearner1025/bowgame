      ...
      const visit=async(directory:string)=>{
        for(const entry of await readdir(directory,{withFileTypes:true})){
          ...
          if(entry.isDirectory())await visit(path);
            ...
            if(runtimeAssets.has(assetPath))this.emitFile({
              type:'asset',
  ...
  return {
    ...
    generateBundle(_options,bundle){
      for(const [name,item] of Object.entries(bundle)){
        const source=item.type==='chunk'?item.code:typeof item.source==='string'?item.source:'';
        const match=forbidden.exec(source);
        if(match)throw new Error(`${name} contains a forbidden remote dependency reference: ${source.slice(Math.max(0,match.index-80),match.index+120)}`);
      }
    },
  };
}
