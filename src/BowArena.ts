import {
    BoxGeometry, BufferGeometry, CanvasTexture, CylinderGeometry,
    DoubleSide, Float32BufferAttribute, Group, InstancedMesh,
    Mesh, MeshStandardMaterial, Object3D, PlaneGeometry, RepeatWrapping,
    SRGBColorSpace, SphereGeometry, Vector3,
} from 'threepipe';

export interface BowObstacle {x: number; z: number; r: number; height: number}
...
export function buildBowArena(): {
    ...
    const add = (geometry: BufferGeometry, material: MeshStandardMaterial, name: string,
    ...
    const rock = (x: number, z: number, radius: number, height: number, collision = true) => {
        const mesh = add(rockGeometry, random() > 0.4 ? stone : stoneDark, 'Weathered granite',
            x, height * 0.36 - 0.12, z, radius, height * 0.65, radius * (0.8 + random() * 0.3));
    ...
    const tree = (x: number, z: number, height: number, collision: boolean) => {
        ...
        for (let tier = 0; tier < 7; tier++) {
            ...
            for (let arm = 0; arm < 6; arm++) {
                ...
                const tip = new Vector3(x + Math.cos(angle) * length,
                    height * fraction - length * 0.15 + random() * 0.25, z + Math.sin(angle) * length);
    ...
    const wall = (x: number, z: number, length: number, angle: number, height = 1.5) => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            add(rockGeometry, stoneDark, 'Broken wall cap', x + Math.cos(angle) * t, height,
                z - Math.sin(angle) * t, 0.5, 0.2 + random() * 0.12, 0.44);
        }
    };
    ...
    const crate = (x: number, z: number, size: number) => {
        ...
        for (const offset of [-0.36, 0.36]) add(box, steel, 'Crate strap', x + size * offset, size / 2, z, 0.075, size + 0.03, size + 0.04);
