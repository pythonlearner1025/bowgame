/**
 * Builds the deterministic procedural forest arena and marks its solid rendered surfaces.
 * It does not build the BVH, run gameplay, or load remote assets.
 */

/*
 * Arena measurements, colors, seeded placements, and texture samples are authored visual data.
 * Keeping them inline preserves the spatial composition and makes the construction calls legible.
 */
/* eslint-disable no-magic-numbers */
/* eslint-disable max-params -- Placement helpers mirror Three.js geometric primitives. */
...
/**
 * Builds the seeded local environment without making asset or network requests.
 *
 * @returns The arena root, legacy obstacles, and deterministic player and bot spawn positions.
 */
export function buildBowArena(): {
  group: Group;
  obstacles: BowObstacle[];
  botSpawns: Vector3[];
  playerSpawn: Vector3;
} {
  const group = new Group();
  group.name = 'K3D_BOW_DEMO_ARENA';
  const obstacles: BowObstacle[] = [];
  let seed = 73429;

  const random = () => {
    // Numerical Recipes LCG parameters and unsigned wrap preserve the original seeded layout.
  ...
  const texture = (kind: 'earth' | 'bark' | 'stone' | 'wood') => {
    ...
    if (!context) {
      throw new Error('Canvas 2D context is required to generate arena textures');
    }
    ...
    for (let i = 0; i < 17000; i++) {
      const noise = random();
      context.fillStyle = `rgba(${noise > 0.5 ? '205,201,166' : '25,29,22'},${0.04 + random() * 0.15})`;
      const x = random() * 512;
      const y = random() * 512;
      const width = kind === 'bark' || kind === 'wood' ? 0.5 + random() * 2 : 1 + random() * 8;
      const height = kind === 'bark' || kind === 'wood' ? 8 + random() * 70 : 1 + random() * 7;
  ...
  // Positional dimensions mirror Three.js constructors and keep the authored placement table
  // visually comparable with its original source.
  const add = (
  ...
  for (let i = 0; i < positions.count; i++) {
    ...
    const distortion =
      1 +
      0.15 * Math.sin(x * 4 + z * 3) * Math.cos(y * 4) +
  ...
  // The fifth argument retains the original optional legacy-cylinder registration switch.
  const rock = (x: number, z: number, radius: number, height: number, collision = true) => {
    const mesh = add(
      rockGeometry,
      random() > 0.4 ? stone : stoneDark,
      'Weathered granite',
      x,
      height * 0.36 - 0.12,
      z,
  ...
  [
    ...
    [-18, 0, 1.7, 2.2],
    [19, -2, 1.8, 2.4],
  ].forEach(([x, z, radius, height]) => rock(x, z, radius, height));
  ...
  needleCanvas.width = 256;
  needleCanvas.height = 256;
  const needleContext = needleCanvas.getContext('2d');

  if (!needleContext) {
    throw new Error('Canvas 2D context is required to generate pine needles');
  }

  // Six scalar arguments follow the Canvas2D line primitive and avoid temporary allocations.
  const stroke = (x1: number, y1: number, x2: number, y2: number, color: string, width: number) => {
  ...
  for (let tier = 0; tier < 14; tier++) {
    ...
    for (const side of [-1, 1]) {
      ...
      for (let needleIndex = 0; needleIndex < 30; needleIndex++) {
        const branchFraction = needleIndex / 30;
        const needleX = 128 + (endX - 128) * branchFraction;
        const needleY = y + 11 + (endY - y - 11) * branchFraction;
        const length = 8 + random() * 15;
        const shade = ['#4f663c', '#657547', '#344b2e', '#7b8453'][Math.floor(random() * 4)];
        stroke(needleX, needleY, needleX + side * length * 0.55, needleY - length, shade, 1.5);
        stroke(needleX, needleY, needleX + side * length, needleY + length * 0.24, shade, 1.5);
      }
    }
  }
  ...
  const tree = (x: number, z: number, height: number, collision: boolean) => {
    ...
    for (let tier = 0; tier < 7; tier++) {
      ...
      for (let arm = 0; arm < 6; arm++) {
        ...
        const tip = new Vector3(
          ...
          height * fraction - length * 0.15 + random() * 0.25,
  ...
  [
    ...
    [7, -21],
    [-10, -20],
  ].forEach(([x, z]) => tree(x, z, 10 + random() * 3, true));
  ...
  // The optional height is retained beside the wall's four authored placement values.
  const wall = (x: number, z: number, length: number, angle: number, height = 1.5) => {
    const mesh = add(box, stone, 'Ruined stone wall', x, height / 2, z, length, height, 0.7);
    mesh.rotation.y = angle;
    const count = Math.ceil(length / 0.8);

    for (let i = 0; i <= count; i++) {
      const offset = (i / count - 0.5) * length;
      obstacles.push({
  ...
  blades.setAttribute(
    ...
    new Float32BufferAttribute(
      [
        -0.1, 0, 0, 0.1, 0, 0, 0.04, 0.65, 0.04, 0, 0, -0.1, 0, 0, 0.1, -0.04, 0.5, 0.04, -0.07, 0,
        -0.07, 0.07, 0, 0.07, 0.09, 0.43, 0.02,
      ],
      3,
    ),
  );
  ...
  for (let i = 0; i < 160; i++) {
    ...
    const scale = 0.06 + random() * 0.15;
    matrix.scale.set(scale * 1.7, scale * 0.5, scale);
    matrix.updateMatrix();
