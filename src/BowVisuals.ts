...
function add(parent:Group,geometry:BufferGeometry,material:MeshStandardMaterial,name:string,position=V(),scale=V(1,1,1)){
    const m=new Mesh(geometry,material);m.name=name;m.position.copy(position);m.scale.copy(scale);m.castShadow=m.receiveShadow=true;parent.add(m);return m;
}
function oval(parent:Group,material:MeshStandardMaterial,name:string,p:Vector3,s:Vector3){return add(parent,new SphereGeometry(1,24,18),material,name,p,s);}
...
function loft(sections:Section[],segments=32,sculpt?:(x:number,y:number,z:number,angle:number)=>Vector3){
    ...
    for(let j=0;j<sections.length;j++)for(let i=0;i<=segments;i++){
        ...
        vertices.push(v.x,v.y,v.z);uv.push(i/segments,j/(sections.length-1));
        if(j&&i){const n=j*(segments+1)+i;if(sections[0][0]<sections[sections.length-1][0])indices.push(n,n-1,n-segments-2,n,n-segments-2,n-segments-1);else indices.push(n,n-segments-2,n-1,n,n-segments-1,n-segments-2);}
    }
...
function skinMaterial(tone:number){
    const size=256,data=new Uint8Array(size*size*4);let seed=9127;
    for(let y=0;y<size;y++)for(let x=0;x<size;x++){
        seed=(Math.imul(seed,1664525)+1013904223)>>>0;
        const grain=(seed/4294967296-.5)*12;
        const mottling=4*Math.sin(x*.11)*Math.sin(y*.08)+3*Math.sin(x*.37+y*.21);
        const j=(y*size+x)*4;data[j]=218+grain+mottling;data[j+1]=198+grain+mottling;data[j+2]=179+grain+mottling;data[j+3]=255;
    }
    const map=new DataTexture(data,size,size,RGBAFormat);map.colorSpace=SRGBColorSpace;map.wrapS=map.wrapT=RepeatWrapping;map.needsUpdate=true;
    const m=new MeshStandardMaterial({color:tone,map,roughness:.82,metalness:0});m.bumpMap=map;m.bumpScale=.0013;return m;
}
function span(m:Mesh,a:Vector3,b:Vector3){m.position.copy(a);m.quaternion.setFromUnitVectors(up,b.clone().sub(a).normalize());m.scale.y=a.distanceTo(b);}
export interface ArmRig {applyPose?:(shoulder:Vector3,elbow:Vector3,grip:Vector3,rotation:Quaternion)=>void;setFingerRelease?:(openness:number)=>void;root:Group;upper:Mesh;lower:Mesh;elbow:Mesh;hand:Group;shoulder:Mesh}
export function makeArm(skin:MeshStandardMaterial,side:number):ArmRig {
    const root=new Group();root.name=side<0?'Left articulated arm':'Right articulated arm';
    ...
    upper.scale.x=upper.scale.z=.85;lower.scale.x=lower.scale.z=.83;
    const elbow=oval(root,skin,'Elbow',V(),V(.045,.052,.047));
    const shoulder=oval(root,skin,'Rounded deltoid',V(),V(.082,.088,.078));
    const hand=new Group();hand.name='Articulated hand';root.add(hand);
    ...
    for(let i=0;i<4;i++){
        const y=.038-i*.025;
        const knuckle=oval(hand,skin,'Finger proximal joint',V(-.029*side,y,-.008),V(.018,.013,.02));knuckle.rotation.y=side*.25;
    ...
    const thumb=oval(hand,skin,'Opposed thumb',V(.019*side,.043,-.014),V(.023,.017,.038));thumb.rotation.y=side*.65;
    return {root,upper,lower,elbow,hand,shoulder};
}
export function poseArm(arm:ArmRig,shoulder:Vector3,elbow:Vector3,wrist:Vector3,handRotation=new Quaternion()){
    span(arm.upper,shoulder,elbow);span(arm.lower,elbow,wrist);arm.elbow.position.copy(elbow);arm.shoulder.position.copy(shoulder);arm.hand.position.copy(wrist);arm.hand.quaternion.copy(handRotation);arm.applyPose?.(shoulder,elbow,wrist,handRotation);
}
interface LegRig {root:Group;shin:Group}
export interface HumanRig {applyPose?:(relaxed:boolean)=>void;root:Group;left:ArmRig;right:ArmRig;legs:LegRig[];skin:MeshStandardMaterial}
...
export function makeHuman(index=0):HumanRig {
    const root=new Group();root.name='Adult male survivor';
    const skin=skinMaterial([0xb88a6d,0x92664f,0xc19a7d,0xa57458,0xb38c6d,0x9e755c][index%6]);
    const sections:Section[]=[[.83,.085,.08],[.9,.162,.119],[.99,.168,.125],[1.07,.139,.105],[1.16,.141,.104],[1.25,.17,.119],[1.34,.206,.128],[1.41,.214,.117],[1.47,.196,.096],[1.51,.13,.079],[1.55,.072,.07]];
    ...
    for(const side of [-1,1]){
        const collar=oval(root,skin,'Clavicle',V(side*.104,1.476,-.071),V(.105,.019,.024));collar.rotation.z=side*.12;
    ...
    const legs:LegRig[]=[];
    for(const side of [-1,1]){
        const leg=new Group();leg.name=side<0?'Left hip joint':'Right hip joint';leg.position.set(side*.092,.91,0);root.add(leg);
        ...
        const shin=new Group();shin.position.set(side*.015,-.43,0);leg.add(shin);
        ...
        for(let toe=0;toe<5;toe++)oval(shin,skin,'Toe',V((toe-2)*.018,-.419,-.159+toe*.007),V(.011-toe*.0007,.019,.03-toe*.002));
    ...
    for(const side of [-1,1]){
        ...
        const brow=oval(root,skin,'Brow ridge',V(side*.03,1.73,-.064),V(.031,.012,.014));brow.rotation.z=side*-.13;
    ...
    const left=makeArm(skin,-1),right=makeArm(skin,1);root.add(left.root,right.root);
    const human={root,left,right,legs,skin};poseHuman(human,0,0,true);return human;
}
export function poseHuman(h:HumanRig,draw:number,walk:number,relaxed=false){
    ...
    h.legs[0].root.rotation.x=walk*.43;h.legs[1].root.rotation.x=-walk*.43;
    h.legs[0].shin.rotation.x=Math.max(0,-walk)*.55;h.legs[1].shin.rotation.x=Math.max(0,walk)*.55;
}
function recurveFinish(){
    const size=128,data=new Uint8Array(size*size*4);let seed=7823;
    for(let y=0;y<size;y++)for(let x=0;x<size;x++){
        seed=(Math.imul(seed,1664525)+1013904223)>>>0;
        const noise=(seed/4294967296-.5)*8,wear=Math.pow(Math.max(0,Math.sin(x*.41+y*.15)),22)*48;
        const i=(y*size+x)*4;data[i]=110+noise+wear;data[i+1]=75+noise+wear*.65;data[i+2]=46+noise+wear*.4;data[i+3]=255;
    }
    const map=new DataTexture(data,size,size,RGBAFormat);map.colorSpace=SRGBColorSpace;map.wrapS=map.wrapT=RepeatWrapping;map.needsUpdate=true;
...
export function makeFieldBow(){
    const group=new Group();group.name='Dark open-limb recurve bow';
    ...
    const body=add(group,riser,finish,'Sculpted recurve riser');body.scale.set(.67,.72,.62);
    ...
    const coils:Vector3[]=[];
    for(let i=0;i<=480;i++){const t=i/480,y=.185+t*.115,angle=t*Math.PI*2*18,radius=.036+Math.sin(t*Math.PI)*.004;coils.push(V(Math.cos(angle)*radius,y,Math.sin(angle)*.025));}
    add(group,new TubeGeometry(new CatmullRomCurve3(coils),480,.0028,6,false),cord,'Tan braided rope riser binding');
    const rails:Mesh[]=[];
    for(const side of [-1,1]){
        const geometry=new BufferGeometry();geometry.setAttribute('position',new Float32BufferAttribute(new Float32Array(65*9*3),3));
        const indices:number[]=[];for(let j=1;j<=64;j++)for(let i=1;i<=8;i++){const n=j*9+i;indices.push(n,n-1,n-10,n,n-10,n-9);}geometry.setIndex(indices);
        const rail=add(group,geometry,finish,'Flexible open limb rail');rail.userData.railSide=side;rails.push(rail);
    }
    const string=new Line(new BufferGeometry().setFromPoints([V(),V(),V()]),new LineBasicMaterial({color:0x6d6574}));string.name='Bowstring anchored to tips and nock';group.add(string);
    group.userData.bowLimb=rails[0];group.userData.bowRails=rails;group.userData.bowString=string;deformBow(group,0);return group;
}
export function bowNock(draw:number){return V(0,.115,.145+.68*draw);}
/** Updates the existing rail buffers; arrow and string retain one shared nock throughout the clip. */
export function deformBow(group:Group,draw:number,vibration=0){
    const rails=group.userData.bowRails as Mesh[]|undefined;if(!rails)return;
    if(group.userData.lastVisualDraw===draw&&group.userData.lastVisualVibration===vibration)return;
    group.userData.lastVisualDraw=draw;group.userData.lastVisualVibration=vibration;
    for(const rail of rails){const p=rail.geometry.getAttribute('position'),side=rail.userData.railSide;
        for(let j=0;j<=64;j++){
            ...
            for(let i=0;i<=8;i++){const a=i/8*Math.PI*2;p.setXYZ(j*9+i,side*gap+Math.cos(a)*radius,y,z+Math.sin(a)*radius*1.35);}
        }
        p.needsUpdate=true;rail.geometry.computeVertexNormals();rail.geometry.computeBoundingSphere();
    }
    const line=group.userData.bowString as Line,sp=line.geometry.getAttribute('position');
    const tip=1.04-.075*draw,z=draw*.18+vibration,nock=bowNock(draw);sp.setXYZ(0,0,-tip,z);sp.setXYZ(1,nock.x,nock.y,nock.z);sp.setXYZ(2,0,tip,z);sp.needsUpdate=true;line.geometry.computeBoundingSphere();
}
export interface BowPose {draw:number;grip:Vector3;roll:number;pull:Vector3;vibration:number;arrowVisible:boolean;phase:string}
/** Seconds are deterministic for inspection, and shared by the actual input/physics path. */
export function sampleBowPose(charge:number,releaseSeconds=-1,releaseCharge=1,time=0,aiming=false):BowPose {
    ...
    if(releaseSeconds>=0&&releaseSeconds<1.05){
        const t=releaseSeconds;draw=smooth(releaseCharge)*(1-smooth(t/.065));
        kick=Math.sin(Math.min(1,t/.11)*Math.PI)*Math.exp(-t*7);
        vibration=Math.sin(t*115)*Math.exp(-t*24)*.035*releaseCharge;
        reload=Math.sin(smooth((t-.16)/.84)*Math.PI);arrowVisible=t>.91;phase=t<.12?'release':t<.86?'nock':'recover';
    }
    ...
    const pull=bowNock(draw);pull.x+=releaseSeconds>=0&&releaseSeconds<.16?smooth(releaseSeconds/.16)*.07:0;
    if(reload){pull.x+=reload*.24;pull.y+=reload*.58;pull.z+=reload*.18;}
