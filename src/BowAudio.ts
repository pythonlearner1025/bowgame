...
const LENGTHS: Record<BowSound, number> = {
  draw: 0.9,
  release: 0.34,
  flight: 0.24,
  whizz: 0.32,
...
export function renderBowSound(kind: BowSound, rate = 24000, seed = 7413): Float32Array {
  ...
  const targetAmplitudeBySound: Record<BowSound, number> = {
    draw: 0.3,
    release: 0.76,
    flight: 0.18,
    whizz: 0.43,
...
export function shouldArrowWhizz(from: AudioPoint, to: AudioPoint, listener: AudioPoint): boolean {
  ...
  const distance = Math.hypot(
    from.x + deltaX * progress - listener.x,
    from.y + deltaY * progress - listener.y,
    from.z + deltaZ * progress - listener.z,
  );
...
export class BowAudio {
  ...
  private played: Record<BowSound, number> = {
    draw: 0,
    release: 0,
    flight: 0,
    whizz: 0,
  ...
  private recordings: Partial<
    Record<RecordedSound, { path: string; status: 'loading' | 'decoded' | 'failed' }>
  > = {};
   ...
   */
  private async loadRecordings(context: AudioContext): Promise<void> {
    await Promise.all(
      (Object.keys(BOW_RECORDED_AUDIO) as RecordedSound[]).map(async (kind) => {
  ...
  getState(): object {
    return {
      ...
      recordings: Object.fromEntries(
        Object.entries(this.recordings).map(([kind, value]) => [kind, { ...value }]),
      ),
      buffers: [...this.buffers].map(([kind, buffer]) => ({
        ...
        path: BOW_RECORDED_AUDIO[kind as RecordedSound] ?? null,
      })),
    };
  }
  /**
   ...
   */
  setListener(position: AudioPoint, right: AudioPoint): void {
   ...
   */
  private spatial(position?: AudioPoint): { gain: number; pan: number } {
    if (!position) {
      return { gain: 1, pan: 0 };
    }
    ...
    return {
      ...
      pan: Math.max(
        ...
        Math.min(
          1,
          (deltaX * this.right.x + deltaY * this.right.y + deltaZ * this.right.z) / (distance || 1),
        ),
      ),
    };
  }
  ...
  private play(kind: BowSound, position?: AudioPoint, loop = false): Voice | undefined {
    ...
    gain.gain.value = loop ? 0 : placement.gain * (BOW_RECORDED_MIX[kind as RecordedSound] ?? 1);
   ...
   * @param position - Optional source position in world-space meters.
   */
  draw(id: number, charge: number, position?: AudioPoint): void {
    ...
    const fadeProgress = Math.max(
      0,
      Math.min(1, (charge - mix.drawFadeStart) / (mix.drawSilentCharge - mix.drawFadeStart)),
    );
    const fade = 1 - fadeProgress * fadeProgress * (3 - 2 * fadeProgress);
    voice.gain.gain.setTargetAtTime(
      placement.gain * (mix.drawBase + charge * mix.drawCharge) * fade,
      timeSeconds,
      mix.drawSmooth,
    );
   ...
   */
  release(position?: AudioPoint, id?: number): void {
    if (id !== undefined) {
      this.draw(id, 0);
    }
   ...
   */
  impact(position: AudioPoint, kind: 'head' | 'body' | 'cover'): void {
    const voice = this.play(kind, position);

    if (kind === 'head' && voice) {
      voice.gain.gain.value = Math.min(
        BOW_AUDIO_MIX.headWorldCap,
        voice.gain.gain.value * BOW_AUDIO_MIX.headWorldBoost,
      );
    }
  }
  ...
  whizz(from: AudioPoint, to: AudioPoint): boolean {
    ...
    const distance = Math.hypot(
      passPoint.x - this.listener.x,
      passPoint.y - this.listener.y,
      passPoint.z - this.listener.z,
    );
