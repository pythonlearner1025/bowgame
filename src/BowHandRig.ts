...
function findNearestSegmentPoint(point: Vector3, segments: ArmSegment[]): Vector3 {
  ...
  for (const segment of segments) {
    ...
    const fraction = Math.max(
      0,
      Math.min(1, point.clone().sub(segment.head).dot(axis) / axis.lengthSq()),
    );
 ...
 */
export function attachFirstPersonArm(
  arm: ArmRig,
  side: number,
  source: HumanAsset | null = getHumanArmSource().asset,
): boolean {
  if (!source) {
    return false;
  }
  ...
  const data = source;
  const letter: 'L' | 'R' = side < 0 ? 'L' : 'R';
  const find = (name: string) =>
    data.bones.findIndex((boneInfo) => boneInfo.name === `${name}.${letter}`);
  ...
  const allowed = new Set(
    data.bones.flatMap((boneInfo, i) =>
      boneInfo.name.endsWith(`.${letter}`) &&
  ...
  const key = (i: number) =>
    `${points.getX(i).toFixed(5)},${points.getY(i).toFixed(5)},${points.getZ(i).toFixed(5)}`;
  ...
  const restUpper = vectorFromArray(data.bones[lower].head).sub(
    vectorFromArray(data.bones[upper].head),
  );
  const restLower = vectorFromArray(data.bones[wrist].head).sub(
  ...
  const orient = (index: number, rotation: Quaternion) =>
    bones[index].quaternion.copy(
      worldRotation(data.bones[index].parent).invert().multiply(rotation),
    );
  const last = {
    ...
    const gripOrientation = roll
      ...
      .multiply(
        handOrientation(
          data,
          letter,
          unit(0.155, 0.837, -0.52),
          new Vector3(-0.607, -0.75, -1.389),
        ),
      );
    const wristOrientation =
      side < 0
        ? gripOrientation.slerp(
            handOrientation(data, letter, unit(0.995, 0.05, -0.1), new Vector3(-0.05, 0.995, 0)),
            loadOpen,
          )
