...
export type BowClientMode = 'solo' | 'online';

/** Resolved mode and endpoint for one page URL. */
export interface BowClientConfig {
  mode: BowClientMode;
 ...
 */
export function isBowLoopbackHost(hostname: string): boolean {
 ...
 */
export function isBlitzAppSubdomain(hostname: string): boolean {
...
function resolveEndpoint(pageUrl: URL): URL {
 ...
 */
export function resolveBowClientConfig(pageUrl: URL, playerName: string): BowClientConfig {
  ...
  const mode: BowClientMode =
    !isSoloForced && (isOnlineForced || isPublishedOnline) ? 'online' : 'solo';
