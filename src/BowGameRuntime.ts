/**
 * Orchestrates bow-deathmatch systems, lifecycle, fixed-step order, snapshots, and test hooks.
 * Individual systems own mutable state transitions, rendering resources, UI, and network glue.
 */
...
import {
  GameState,
  type BowGameConfig,
  type BowGameRuntimeOptions,
  type CollisionTestAction,
  type InspectOptions,
} from './GameState.js';
...
export class BowGameRuntime {
  ...
  readonly network: NetworkGlue;
  readonly bow: BowController;
  readonly entry: EntryOverlay;
  readonly hud: Hud;
  ...
  constructor(
    ...
    this.arrows = new ArrowSystem(this.state, this.world, () => this.requiredConfig, {
      ...
      applyDamage: (victim, damage, owner, isHead) =>
      ...
      sendHit: (targetId, arrowId, damage, isHead) =>
        this.network.sendHit(targetId, arrowId, damage, isHead),
    });
    this.bots = new BotSystem(this.state, this.world, () => this.requiredConfig, {
      fire: (position, direction, owner, charge) =>
        this.arrows.fire(position, direction, owner, charge),
    });
    this.remotePlayers = new RemotePlayers(this.state, this.world, this.bots, (slot) =>
      this.world.getSlotSpawn(this.requiredConfig, slot),
    );
    ...
    this.network = new NetworkGlue(this.state, options.session ?? null, {
      ...
      spawnArrow: (position, velocity, spawnOptions) =>
        this.arrows.spawn(position, velocity, spawnOptions),
  ...
  private validateInspection(
    ...
    values: { view: string; draw: number; release: number; orbit: number },
  ): void {
    ...
    if (
      params.flightSeconds !== undefined &&
  ...
  private applyFlightInspection(flightSeconds?: number): void {
