...
export class BowGameRuntime {
  ...
  private playerName = '';
  private deathFeed: DeathFeedEntry[] = [];
  ...
  private onNetwork(message: ServerMessage | null, snapshot: NetSnapshot) {
    ...
    if (message?.type === 'death') {
      this.addDeath(
        this.getNetworkPlayerName(message.killerId, snapshot),
        this.getNetworkPlayerName(message.playerId, snapshot),
      );
  ...
  private getNetworkPlayerName(playerId: string, snapshot: NetSnapshot): string {
  ...
  private applyEnteredPlayerName(): void {
  ...
  private addDeath(killer: string, victim: string): void {
  ...
  private setHudAttribute(id: string, attribute: string, value: string): void {
  ...
  private createHudElement(id: string, parent: HTMLElement, style = '', text = ''): HTMLDivElement {
  ...
  private addHudStyles(overlay: HTMLElement): void {
...
`;
  ...
  private makeStandingsHud(overlay: HTMLElement): void {
    ...
    boardPlate.append(
      this.createHudElement('score', boardPlate),
      this.createHudElement('board', boardPlate),
    );
  ...
  private makeHealthHud(overlay: HTMLElement): void {
  ...
  private makeNameField(modal: HTMLElement): void {
    ...
    const nameField = this.createHudElement(
      'nameField',
      modal,
    ...
    input.onkeydown = (event) => {
      if (event.key === 'Enter') {
        event.preventDefault();
        (this.hud.button as HTMLButtonElement).click();
      }
    };
  }
  ...
  private makeEntryButtons(modal: HTMLElement): void {
  ...
  private makeEntryModal(overlay: HTMLElement): void {
    const modal = this.createHudElement(
      'modal',
      overlay,
    ...
    const title = this.createHudElement(
      'title',
      modal,
    ...
    const description = this.createHudElement(
      'description',
      modal,
  ...
  private makeHud(): void {
    ...
    this.createHudElement(
      'damage',
      overlay,
      'position:absolute;inset:0;box-shadow:inset 0 0 100px 30px #a02b22;opacity:0;',
    );
  ...
  private getOnlineModalDescription(status: NetSnapshot['status'], limit: number): string {
    if (status === 'full') {
  ...
  private updateOnlineModal(limit: number): void {
  ...
  private updateHud(): void {
    ...
    this.setHudStyle(
      'healthbar',
      '--health-color',
      health < LOW_HEALTH_THRESHOLD ? '#a14f37' : '#788452',
    );
