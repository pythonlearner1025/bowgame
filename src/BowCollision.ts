import {Box3, BufferGeometry, DoubleSide, Float32BufferAttribute, Line3, Matrix4, Ray, Vector3, type Group, type Mesh, type InstancedMesh} from 'three';
...
export class BowCollision {
    ...
    private closedTriangles: Uint8Array;
    private line = new Line3();
    private box = new Box3();
    private trianglePoint = new Vector3();
    private capsulePoint = new Vector3();
    ...
    constructor(arena: Group) {
        const start = performance.now(), vertices: number[] = [], closed: number[] = [];
        let count = 0;
        ...
        arena.traverse(object => {
            ...
            if (!mesh.isMesh || mesh.userData.bowSolid !== true) return;
            ...
            for (let copy = 0; copy < copies; copy++) {
                ...
                if (instanced.isInstancedMesh) { instanced.getMatrixAt(copy, instance); world.multiply(instance); }
        ...
        if (!vertices.length) throw new Error('Arena has no solid collision geometry');
    ...
    move(feet: Vector3, velocity: Vector3, dt: number, radius = PLAYER_RADIUS) {
        ...
        this.steps++; this.totalMs += ms; this.maxMs = Math.max(this.maxMs, ms);
        return grounded;
    }

    resolve(feet: Vector3, velocity: Vector3, radius = PLAYER_RADIUS) {
        let grounded = false;
        for (let pass = 0; pass < 8; pass++) {
            ...
            this.bvh.shapecast({
                ...
                intersectsTriangle: triangle => {
                    ...
                    if (distance > 1e-9) this.normal.divideScalar(distance);
                    else this.normal.copy(this.faceNormal);
                    if (this.faceNormal.y >= WALKABLE_Y && this.normal.y >= WALKABLE_Y && velocity.y <= .01) grounded = true;
                    ...
                    if (this.faceNormal.y < WALKABLE_Y && this.normal.y > 0 && this.normal.y < .999) {
                        ...
                        this.normal.y = 0; this.normal.divideScalar(horizontal); depth /= horizontal;
                    }
                    ...
                    if (into < 0) velocity.addScaledVector(this.normal, -into);
                    return false;
                },
            });
            if (correction < SKIN * .5) break;
        }
        return grounded;
    }

    penetration(feet: Vector3, radius = PLAYER_RADIUS) {
        this.setCapsule(feet, radius);
        let depth = 0;
    ...
    spawn(wanted: Vector3, radius = PLAYER_RADIUS) {
        ...
        for (let ring = 0; ring <= 16; ring++) {
            ...
            for (let i = 0; i < samples; i++) {
                ...
                for (const hit of this.bvh.raycast(ray, DoubleSide)) {
                    if (hit.face!.normal.y < WALKABLE_Y) continue;
                    const feet = hit.point.clone(); feet.y += SKIN;
                    if (this.penetration(feet, radius) > SKIN) continue;
                    ...
                    if (above && above.face!.normal.y > 0) continue;
                    candidates.push(feet);
                }
            }
            candidates.sort((a, b) => a.distanceToSquared(wanted) - b.distanceToSquared(wanted));
            if (candidates.length && candidates[0].distanceTo(wanted) < (ring + 1) * .25) return candidates[0];
        }
        if (!candidates.length) throw new Error('No free supported spawn near requested position');
        return candidates[0];
    }
