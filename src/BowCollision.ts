...
interface CollectedCollisionGeometry {
  vertices: number[];
  closedTriangles: number[];
  solidMeshes: number;
}

/** The earliest static-world hit along a segment. */
export interface BowCollisionHit {
  t: number;
  point: Vector3;
  normal: Vector3;
}

/** Runtime construction and capsule-query measurements for collision diagnostics. */
export interface BowCollisionStats {
  buildMs: number;
  triangles: number;
  solidMeshes: number;
...
function collectMeshInstance(
  ...
  for (
    let triangleIndex = firstIndex;
    triangleIndex + 2 < finalIndex;
    triangleIndex += VERTICES_PER_TRIANGLE
  ) {
...
function collectMarkedGeometry(arena: Group, marker: string): CollectedCollisionGeometry {
  const vertices: number[] = [];
  const closedTriangles: number[] = [];
...
export class BowCollision {
  ...
  readonly solidMeshes: number;
  readonly triangles: number;
  readonly barrierGeometry = new BufferGeometry();
  readonly barrierBvh: MeshBVH | null;
  readonly barrierTriangles: number;

  private readonly closedTriangles: Uint8Array;
  private readonly capsuleLine = new Line3();
  ...
  private readonly segmentRay = new Ray();
  private readonly capsuleAxisRay = new Ray(new Vector3(), new Vector3(0, 1, 0));
  private readonly segmentDirection = new Vector3();
  private readonly supportFeet = new Vector3();
  ...
  constructor(arena: Group) {
    ...
    this.geometry.setAttribute(
      'position',
      new Float32BufferAttribute(collected.vertices, POSITION_COMPONENTS),
    );
    ...
    console.info(
      `[BowCollision] ${this.triangles} triangles / ${this.solidMeshes} solids; ` +
   ...
   */
  segment(start: Vector3, end: Vector3): BowCollisionHit | null {
  ...
  private setCapsule(feet: Vector3, radius: number): void {
   ...
   */
  move(feet: Vector3, velocity: Vector3, dt: number, radius = PLAYER_RADIUS): boolean {
    const startedAt = performance.now();
   ...
   */
  resolve(feet: Vector3, velocity: Vector3, radius = PLAYER_RADIUS): boolean {
    let isGrounded = false;
   ...
   */
  hasSupport(feet: Vector3, radius = PLAYER_RADIUS): boolean {
  ...
  private resolvePass(
    ...
    velocity: Vector3,
    radius: number,
  ): { isGrounded: boolean; maxCorrection: number } {
    this.setCapsule(feet, radius);
    let isGrounded = false;
    let maxCorrection = 0;

    const contactVisitor = {
      intersectsBounds: (bounds: Box3) => bounds.intersectsBox(this.capsuleBounds),
  ...
  private resolveTriangleContact(
    triangle: ExtendedTriangle,
    feet: Vector3,
    velocity: Vector3,
    radius: number,
  ): { isGrounded: boolean; correction: number } {
    ...
    const needsHorizontalProjection =
      this.faceNormal.y < WALKABLE_Y &&
   ...
   */
  penetration(feet: Vector3, radius = PLAYER_RADIUS): number {
    this.setCapsule(feet, radius);
    ...
    const penetrationVisitor = {
      ...
      intersectsTriangle: (triangle: ExtendedTriangle) => {
        ...
        const crossing = this.capsuleAxisRay.intersectTriangle(
          triangle.a,
          triangle.b,
          triangle.c,
          false,
   ...
   */
  spawn(wanted: Vector3, radius = PLAYER_RADIUS): Vector3 {
    const candidates: Vector3[] = [];

    for (let ring = 0; ring <= SPAWN_RING_COUNT; ring += 1) {
      this.collectSpawnRing(candidates, wanted, radius, ring);
      candidates.sort(
        (first, second) => first.distanceToSquared(wanted) - second.distanceToSquared(wanted),
      );
  ...
  private makeSpawnCandidate(
    hit: ReturnType<MeshBVH['raycast']>[number],
    radius: number,
  ): Vector3 | null {
    ...
    const insideRay = new Ray(
      feet.clone().add(new Vector3(0, PLAYER_HEIGHT / 2, 0)),
      new Vector3(0, 1, 0),
    );
  ...
  private buildBarrierBvh(vertices: number[]): MeshBVH | null {
    if (vertices.length === 0) {
      return null;
    }

    this.barrierGeometry.setAttribute(
      'position',
      new Float32BufferAttribute(vertices, POSITION_COMPONENTS),
    );
