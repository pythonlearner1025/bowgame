import {Object3DComponent,literalStrings,type ComponentDefn,type ViewerEventMap} from 'threepipe';
...
export class BowGameComponent extends Object3DComponent {
    ...
    start(){
        ...
        if(!DIFFICULTIES.includes(difficulty))throw new Error('difficulty must be easy, normal, or hard');
        ...
        const runtime=new BowGameRuntime(this.ctx.viewer,config,arena.group,()=>!this.ctx.ecp.running,true,(window as any).__KITE_BOW_SESSION__??null,(window as any).__KITE_BOW_TELEMETRY__??null);
        this.runtime=runtime;
        (window as any).__KITE_BOW_GAME__=this;
        void runtime.start().then(()=>{
            if(generation!==this.generation||this.runtime!==runtime)runtime.stop();
        }).catch(error=>{
    ...
    update({deltaTime,time}:ViewerEventMap['preFrame']){
    ...
    stop(){
        ...
        this.runtime=null;
        runtime?.stop();
        if((window as any).__KITE_BOW_GAME__===this)delete (window as any).__KITE_BOW_GAME__;
    }
