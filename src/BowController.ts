 ...
 */
import { Line, Quaternion, Vector3, type Camera, type ThreeViewer } from 'threepipe';
import { createArrowModel, type ArrowSystem } from './ArrowSystem.js';
...
/** External camera and audio operations needed by the bow state machine. */
export interface BowControllerCallbacks {
  updateCamera: () => void;
  getAudio: () => BowAudio | null;
}

/** Construction dependencies for the local bow state machine. */
export interface BowControllerOptions {
  viewer: ThreeViewer;
  state: GameState;
  world: GameWorld;
  arrows: ArrowSystem;
  callbacks: BowControllerCallbacks;
}

interface ViewPoseContext {
  camera: Camera;
  pose: ReferencePose;
...
export class BowController {
  ...
  private callbacks: BowControllerCallbacks;
   ...
   */
  constructor(options: BowControllerOptions) {
   ...
   */
  stepTransitions(dt: number): void {
    this.state.aimBlend += Math.max(
      -dt / 0.4,
      Math.min(dt / 0.4, Number(this.state.aiming) - this.state.aimBlend),
    );
    this.stepRelease(dt);
    this.stepCancel(dt);
  ...
  private stepCancel(dt: number): void {
   ...
   */
  firePlayer(pose: ReferencePose, charge: number): void {
  ...
  private sampleReleasePose(): ReferencePose {
  ...
  private sampleCancelPose(): ReferencePose {
    const from = this.state.cancelFrom as ReferencePose;
   ...
   */
  updateViewModel(camera: Camera): void {
  ...
  private getViewPose(release: number): ReferencePose {
    if (this.state.preview?.referenceTime !== undefined) {
      return sampleReferenceTimeline(this.state.preview.referenceTime);
    }

    if (this.state.preview) {
      return sampleReferenceAction(
        this.state.preview.draw ?? 0,
        release,
        (this.state.preview.aim ?? this.state.aiming) ? 1 : 0,
      );
    }
  ...
  private poseHands(context: ViewPoseContext): void {
  ...
  private applyArrowPose(context: ViewPoseContext): void {
    const arrow = referenceArrow(context.pose);
    ...
    if (context.pose.arrowVisible && context.pose.arrowSeated) {
      ...
      const string = this.state.bow.userData.bowString as Line;
   ...
   */
  updateDrawAudio(isActive: boolean): void {
