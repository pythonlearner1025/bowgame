...
export function preloadHumanAsset(){
    return pending??=Promise.all([
        fetch(base+'male-adult-rigged.json').then(r=>{if(!r.ok)throw new Error('Adult model unavailable');return r.json();}),
...
function smoothAnatomyNormals(g:BufferGeometry){
    g.computeVertexNormals();const p=g.getAttribute('position'),n=g.getAttribute('normal'),sums=new Map<string,Vector3>();
    const key=(i:number)=>`${p.getX(i).toFixed(5)},${p.getY(i).toFixed(5)},${p.getZ(i).toFixed(5)}`;
    for(let i=0;i<p.count;i++){const k=key(i),sum=sums.get(k)??new Vector3();sum.add(new Vector3().fromBufferAttribute(n,i));sums.set(k,sum);}
    for(let i=0;i<p.count;i++){const normal=sums.get(key(i))!.clone().normalize();n.setXYZ(i,normal.x,normal.y,normal.z);}n.needsUpdate=true;
}
/** Keep the procedural rig's public IK controls, replacing its surface with a connected GPU-skinned body. */
export function attachHumanAsset(human:HumanRig,index:number,source=asset){
    if(!source)return false;
    const data=source;
    human.root.traverse((o:any)=>{if(o.isMesh&&o.name!=='Minimal weathered modesty wrap')o.visible=false;});
    const geometry=new BufferGeometry();geometry.setAttribute('position',new Float32BufferAttribute(data.positions,3));geometry.setAttribute('uv',new Float32BufferAttribute(data.uvs,2));geometry.setAttribute('skinIndex',new Uint16BufferAttribute(data.skinIndex,4));geometry.setAttribute('skinWeight',new Float32BufferAttribute(data.skinWeight,4));geometry.setIndex(data.indices);smoothAnatomyNormals(geometry);
    ...
    const surface=new SkinnedMesh(geometry,material);surface.name='CC0 connected adult male anatomy';surface.castShadow=surface.receiveShadow=true;surface.frustumCulled=false;
    ...
    const visual=new Group();visual.name='Grounded survivor surface';visual.position.y=-.04;human.root.add(visual);const cover=human.root.getObjectByName('Minimal weathered modesty wrap');if(cover)visual.add(cover);visual.add(surface);visual.updateMatrixWorld(true);surface.updateMatrixWorld(true);surface.bind(new Skeleton(bones));
    if(data.eyes){
        const e=data.eyes,g=new BufferGeometry();g.setAttribute('position',new Float32BufferAttribute(e.positions,3));g.setAttribute('uv',new Float32BufferAttribute(e.uvs,2));g.setAttribute('skinIndex',new Uint16BufferAttribute(e.skinIndex,4));g.setAttribute('skinWeight',new Float32BufferAttribute(e.skinWeight,4));g.setIndex(e.indices);g.computeVertexNormals();
        const eyes=new SkinnedMesh(g,new MeshStandardMaterial({map:eyeTexture??null,roughness:.35}));eyes.name='Anatomical eyes';eyes.frustumCulled=false;visual.add(eyes);eyes.bind(surface.skeleton,surface.bindMatrix);
    }
    ...
    const poseBone=(index:number,worldRotation:Quaternion)=>{
        ...
        if(parent>=0){const chain:number[]=[];let p=parent;while(p>=0){chain.unshift(p);p=data.bones[p].parent;}for(const b of chain)parentRotation.multiply(bones[b].quaternion);}
        bones[index].quaternion.copy(parentRotation.invert().multiply(worldRotation));
    };
    const localHead=(index:number)=>surface.worldToLocal(bones[index].getWorldPosition(new Vector3()));
    const solveArm=(side:string,goal:Vector3,guide:Vector3,relaxed:boolean,roll:Quaternion)=>{
        const upper=find('upperarm01.'+side),lower=find('lowerarm01.'+side),wrist=find('wrist.'+side);
        if(upper<0||lower<0||wrist<0)return;
        ...
        const target=goal.clone().add(relaxed?new Vector3(0,.035,0):(side==='L'?new Vector3(-.038,-.026,.106):new Vector3(.010,-.023,.145)).applyQuaternion(roll));target.y+=.04;
        const direction=target.clone().sub(shoulder),distance=Math.min(direction.length(),length1+length2-.001);direction.normalize();
    ...
    human.applyPose=(relaxed:boolean)=>{
        ...
        for(let i=0;i<2;i++){
            ...
            if(upper>=0)bones[upper].quaternion.setFromAxisAngle(new Vector3(1,0,0),human.legs[i].root.rotation.x);
            if(lower>=0)bones[lower].quaternion.setFromAxisAngle(new Vector3(1,0,0),human.legs[i].shin.rotation.x);
        }
        surface.updateMatrixWorld(true);
    };
    human.applyPose(true);return true;
}
