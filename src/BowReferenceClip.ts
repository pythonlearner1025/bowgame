...
/** A sampled first-person bow pose in normalized screen and local weapon coordinates. */
export interface ReferencePose {
  ...
  rightOpen: number;
  phase: string;
  arrowTip: [number, number];
  arrowNear: [number, number];
  upperTip?: [number, number];
}
interface ReferenceKeyframe {
  timeSeconds: number;
  pose: ReferencePose;
}
const pose = (
  grip: ReferencePose['grip'],
  rotation: ReferencePose['rotation'],
  tension = 0,
  extra: Partial<ReferencePose> = {},
): ReferencePose => ({
  grip,
  rotation,
  tension,
...
const mix = (start: number, end: number, fraction: number) => start + (end - start) * fraction;
 ...
 */
export function blendReferencePoses(
  start: ReferencePose,
  end: ReferencePose,
  fraction: number,
): ReferencePose {
  return {
    ...
    grip: start.grip.map((value, i) => mix(value, end.grip[i], fraction)) as ReferencePose['grip'],
    rotation: start.rotation.map((value, i) =>
      mix(value, end.rotation[i], fraction),
    ) as ReferencePose['rotation'],
    right: start.right.map((value, i) =>
      mix(value, end.right[i], fraction),
    ) as ReferencePose['right'],
    tension: mix(start.tension, end.tension, fraction),
    arrowTip: start.arrowTip.map((value, i) => mix(value, end.arrowTip[i], fraction)) as [
      number,
      number,
    ],
    arrowNear: start.arrowNear.map((value, i) => mix(value, end.arrowNear[i], fraction)) as [
      number,
      number,
    ],
...
function track(keyframes: ReferenceKeyframe[], seconds: number): ReferencePose {
  ...
  for (let i = 1; i < keyframes.length; i++) {
    ...
    if (seconds <= current.timeSeconds) {
      ...
      const linearFraction =
        (seconds - previous.timeSeconds) / (current.timeSeconds - previous.timeSeconds);
  ...
  if (release >= 0) {
    ...
    const recoil = {
      ...
      grip: [
        ...
        from.grip[1] + 0.032,
        from.grip[2] - 0.015,
      ] as ReferencePose['grip'],
      tension: 0,
      arrowVisible: false,
      phase: 'release',
      rightVisible: false,
      arrowSeated: false,
      right: [0.69, 1.12, 0.26] as ReferencePose['right'],
    };
 ...
 */
export function referenceScreenPoint(
  horizontalCoordinate: number,
  verticalCoordinate: number,
  depth: number,
  fieldOfViewDegrees = 76,
): Vector3 {
  const height = 2 * depth * Math.tan((fieldOfViewDegrees * Math.PI) / 360);

  return new Vector3(
    ((horizontalCoordinate - 0.5) * height * 16) / 9,
    (0.5 - verticalCoordinate) * height,
    -depth,
  );
}
 ...
 */
export function referenceRotation(pose: ReferencePose): Quaternion {
  if (pose.upperTip) {
...
export function referenceArrow(
  pose: ReferencePose,
  length = 0.9405,
): { nock: Vector3; tip: Vector3; rotation: Quaternion } {
