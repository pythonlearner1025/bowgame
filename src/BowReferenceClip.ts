...
export interface ReferencePose {
    grip:[number,number,number];rotation:[number,number,number];tension:number;
    right:[number,number,number];rightVisible:boolean;arrowVisible:boolean;arrowSeated:boolean;
    leftOpen:number;rightOpen:number;phase:string;arrowTip:[number,number];arrowNear:[number,number];upperTip?:[number,number];
}
type Key={t:number;pose:ReferencePose};
const pose=(grip:ReferencePose['grip'],rotation:ReferencePose['rotation'],tension=0,extra:Partial<ReferencePose>={}):ReferencePose=>({grip,rotation,tension,right:[.76,1.22,.22],rightVisible:false,arrowVisible:false,arrowSeated:true,leftOpen:0,rightOpen:0,phase:'carry',arrowTip:[.615,.608],arrowNear:[.745,1.10],...extra});
...
function blend(a:ReferencePose,b:ReferencePose,t:number):ReferencePose {
    return {...(t<.5?a:b),grip:a.grip.map((v,i)=>mix(v,b.grip[i],t)) as ReferencePose['grip'],rotation:a.rotation.map((v,i)=>mix(v,b.rotation[i],t)) as ReferencePose['rotation'],right:a.right.map((v,i)=>mix(v,b.right[i],t)) as ReferencePose['right'],tension:mix(a.tension,b.tension,t),arrowTip:a.arrowTip.map((v,i)=>mix(v,b.arrowTip[i],t)) as [number,number],arrowNear:a.arrowNear.map((v,i)=>mix(v,b.arrowNear[i],t)) as [number,number],leftOpen:mix(a.leftOpen,b.leftOpen,t),rightOpen:mix(a.rightOpen,b.rightOpen,t)};
}
function track(keys:Key[],seconds:number){
    if(seconds<=keys[0].t)return {...keys[0].pose};
    for(let i=1;i<keys.length;i++)if(seconds<=keys[i].t){const u=(seconds-keys[i-1].t)/(keys[i].t-keys[i-1].t),s=u*u*(3-2*u);return blend(keys[i-1].pose,keys[i].pose,s);}
    return {...keys[keys.length-1].pose};
}
export function sampleReferenceAction(charge:number,release=-1,aim=0):ReferencePose {
    if(release>=0){
        const from=blend(ready,aimed,aim),recoil={...from,grip:[from.grip[0]+.012,from.grip[1]+.032,from.grip[2]-.015] as ReferencePose['grip'],tension:0,arrowVisible:false,phase:'release',rightVisible:false,arrowSeated:false,right:[.69,1.12,.26] as ReferencePose['right']};
    ...
    if(charge<=0)return {...carry};
    const result=track(load,Math.min(1,charge)*BOW_LOAD_DRAW_SECONDS);
    if(charge>.82)return blend(result,{...aimed,tension:result.tension},aim*Math.min(1,(charge-.82)/.18));
    return result;
}
...
export function sampleReferenceTimeline(time:number):ReferencePose {
    if(time<.78)return {...carry,grip:[.568+Math.sin(time*9)*.015,.949+Math.sin(time*12)*.008,.65]};
    if(time<1.34)return track([{t:.78,pose:carry},{t:1,pose:pose([.335,.930,.65],[.05,.05,-.81])},{t:1.34,pose:carry}],time);
    if(time<1.96)return {...carry};
    if(time<3.13)return sampleReferenceAction((time-1.96)/BOW_LOAD_DRAW_SECONDS);
    if(time<3.53)return track([{t:3.13,pose:ready},{t:3.4,pose:{...aimed,grip:[.5464,.6620,.84],rotation:[0,0,-.410],arrowTip:[.5,.5],arrowNear:[.5,1.10]}},{t:3.53,pose:aimed}],time);
    if(time<3.97)return track([{t:3.53,pose:aimed},{t:3.80,pose:{...carry,tension:0,arrowVisible:true,phase:'cancel'}},{t:3.97,pose:{...carry,phase:'carry'}}],time);
    if(time<6.22)return {...carry,grip:[.565+Math.sin(time*8)*.015,.958+Math.sin(time*10)*.009,.65]};
    if(time<6.48)return track([{t:6.22,pose:carry},{t:6.36,pose:{...ready,grip:[.57,.77,.81],tension:.55}},{t:6.48,pose:aimed}],time);
    if(time<8.466667)return {...aimed};
    if(time<9.10)return sampleReferenceAction(0,time-8.466667,1);
    if(time<9.70)return track([{t:9.1,pose:carry},{t:9.3,pose:pose([.404,.907,.56],[-.25,-.30,.46],0,{leftOpen:.95,upperTip:[.229,.273],phase:'load'})},{t:9.515,pose:{...load[3].pose,arrowVisible:false,rightVisible:false}},{t:9.55,pose:{...load[4].pose,grip:[.715,.968,.65]}},{t:9.60,pose:{...load[4].pose,grip:[.698,.952,.65],arrowTip:[.698,.693],arrowNear:[.746,1.10]}},{t:9.70,pose:load[5].pose}],time);
    return sampleReferenceAction((time-9.10)/BOW_LOAD_DRAW_SECONDS);
}
export function referenceScreenPoint(u:number,v:number,depth:number,fov=76){
    const height=2*depth*Math.tan(fov*Math.PI/360);
    return new Vector3((u-.5)*height*16/9,(.5-v)*height,-depth);
}
export function referenceRotation(pose:ReferencePose){
    if(pose.upperTip){
...
export function referenceArrow(pose:ReferencePose,length=.9405){
