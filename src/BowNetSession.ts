...
export class BowNetSession {
    ...
    constructor(private readonly transport:Transport,private name:string){this.name=cleanPlayerName(name);}
    ...
    private onTransport=(event:TransportEvent)=>{
        ...
        switch(message.type){
            ...
            case'leave':this.players.delete(message.playerId);delete this.scores[message.playerId];break;
            case'pong':this.latencyMs=Math.max(0,performance.now()-message.sentAt);break;
        }
        this.emit(message);
    };
}
