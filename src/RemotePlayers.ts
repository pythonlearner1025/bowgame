...
import { createArrowModel, type ArrowCandidate } from './ArrowSystem.js';
...
import type { GameState, RemotePlayerState } from './GameState.js';
import { disposeGroup, type GameWorld } from './GameWorld.js';
import type { NetSnapshot } from './BowNetSession.js';
...
export class RemotePlayers {
  ...
  constructor(
    ...
    private getSlotSpawn: (slot: number) => Vector3,
  ) {}
   ...
   */
  create(id: string, name: string, slot: number): RemotePlayerState {
   ...
   */
  sync(snapshot: NetSnapshot): void {
    ...
    for (const slot of snapshot.players) {
      ...
      wanted.add(slot.id);
      const player =
        this.state.remotePlayers.get(slot.id) ?? this.create(slot.id, slot.name, slot.slot);
   ...
   */
  getHitCandidates(excludedPlayerId?: string): ArrowCandidate[] {
