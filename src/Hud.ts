/**
 * Owns active-play health, leaderboard, hit feedback, and death-feed DOM with cached writes.
 * It delegates name entry and modal flow to EntryOverlay and never changes simulation rules.
 */
import type { ThreeViewer } from 'threepipe';
import type { EntryOverlay } from './EntryOverlay.js';
import type { BowGameConfig, GameState, LeaderboardEntry } from './GameState.js';
...
export class Hud {
  ...
  private addStyles(overlay: HTMLElement): void {
...
`;
  ...
  private makeStandings(overlay: HTMLElement): void {
    ...
    boardPlate.append(
      this.createElement('score', boardPlate),
      this.createElement('board', boardPlate),
    );
  ...
  private makeHealth(overlay: HTMLElement): void {
  ...
  private getLeaderboardEntries(isOnline: boolean): LeaderboardEntry[] {
    if (isOnline) {
  ...
  private updateLeaderboard(isOnline: boolean): void {
  ...
  private updateDeathFeed(): void {
    const activeEntries = this.state.deathFeed.filter(
      (entry) => entry.expiresAtSeconds > this.state.elapsed,
    );
